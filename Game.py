...
class GameRunner(Thread):
    ...
    def handleShot(self, data):
        ...
        if maybeShoot is not None:
            ...
            if maybeCoord is not None:
                return (maybeCoord, *self.otherClient()["board"].shoot(
                    maybeCoord))  # Here the bool doesn't indicate success of the command, but hit or miss of the shot. The coord being parsed is already enough to guarantee a valid shot
    ...
    def doBattle(self):
        while not self.end:
            for t in range(0, 2):
                ...
                result = "RESULT MISS" if not hit else (
                            "RESULT HIT" + (" YOUSUNKMY " + sunk.name.upper() if sunk is not None else ""))
        ...
        try:
            ...
            if self.clientA["board"].viz is not None:
                self.clientA["board"].viz.syncUpdate(Visualiser.updateTitle,
                                                     [self.clientA["name"], self.clientB["name"]])
